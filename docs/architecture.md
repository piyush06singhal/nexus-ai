...
- **`providers/`** — concrete adapters. `MockProvider` is deterministic and key-free (the default); `OpenAIProvider` is a **real** Chat Completions client (httpx, bounded retry/backoff, keyless fallback, native SSE streaming) and `AnthropicProvider` is a **real** Messages client (`x-api-key`, `anthropic-version`, system-prompt hoist, native SSE) — both share transport, retry policy, and error mapping via `_client.py`, and both degrade to deterministic output when no key is set. Adapters activate automatically when a key is configured (`settings.openai_api_key` / `settings.anthropic_api_key`). A model name may carry a provider prefix (`anthropic/claude-sonnet-5`) that the runtime resolves by walking the provider registry (mock path untouched). `structured_output` on OpenAI is **protocol-level**: a `response_format: json_schema` (strict-schema mode) is sent so the API enforces the target Pydantic schema, then the reply is validated; an API that rejects `json_schema` parameter (older models / compatible endpoints) degrades to the base JSON-parse+validate path, keyless/stub mode returns a validated default instance, and Anthropic mirrors the same keyless contract (its native enforcement would map to the tool-use JSON mode — future work).
- **`registry.py`** — resolves provider by name (`get_provider("openai")`), and underpins the provider-prefix model-name protocol in `runtime.py` (`_split_provider_prefix`).
...
- **`retrieval.py`** — `HybridRetriever.retrieve(query, *, namespace, owner_id, memory_types, top_k, min_score, include_expired, policy)` filters by namespace/owner/type/status (excluding expired), then scores each candidate by weighted **semantic** (cosine when embeddings exist) + **keyword** (Dice coefficient) + **recency** (exponential half-life decay) + **importance** + **confidence**, applies an optional per-type multiplier, and returns the top-k ranked `MemoryRetrievalResult` with a per-result **`breakdown`** for observability. On PostgreSQL it prefers a **native pgvector path** (below): a bounded `ORDER BY embedding_vector <=> :qv` prefetch seeded from the HNSW index, then the same blended scoring — with `last_used_pgvector` set on the retriever so callers can see which path ranked a query.
- **`pgvector.py`** — the PostgreSQL-only bridge (Tier B). `pgvector_enabled(bind)` probes (once per engine, cached) whether the engine is PostgreSQL **and** the migration-created `embedding_vector vector(1536)` column exists; `sync_vector(db, id, vector)` mirrors a portable embedding into the native column on write; `nearest_neighbours_clause()` + `vector_literal()` feed the native ordering. Every call is a no-op on SQLite or absent extension, so the Python-cosine path is always the fallback.
...
- **Model / migration** — single `memories` table (`Migration 0005`) with `namespace`, `type`, `owner_type`/`owner_id`, `status`, `source_type`/`source_id`, `content`, `summary`, `metadata_json`, `embedding` (text-serialized vector), `confidence`, `importance`, `access_count`, `last_accessed_at`, `expires_at`, and timestamps, plus composite indexes for namespace-scoped queries, expiry cleanup, and recency. `Migration 0015` (PostgreSQL only, a no-op on SQLite) adds `embedding_vector vector(1536)` and a `vector_cosine_ops` HNSW index — the dev/CI Postgres is the `pgvector/pgvector` image so the extension is present. The ORM model stays column-free: the native column is written via `sync_vector` raw `UPDATE`s and read via native `<=>`.
...
- **Design accommodation:** A provider-independent memory system in `app/memory/`: an `EmbeddingProvider` abstraction (mock for tests/local, OpenAI + Anthropic-adjacent real embeddings when keyed), a `HybridRetriever` that scores memories by weighted semantic + keyword + recency + importance + confidence, `RetrievalPolicy`/`WritePolicy` for budgets/thresholds/TTL/dedup, and `extract_memories_from_execution` that auto-mines episodic/semantic/procedural memories from completed runs. Stored in a single `memories` table with 5 types, namespace isolation, ownership, and TTL expiry. The runtime retrieves relevant memories before building context and persists new ones after execution — both best-effort. A per-result `breakdown` keeps ranking observable.
- **Interface points:** `EmbeddingProvider.embed()`, `HybridRetriever.retrieve()`, `RetrievalPolicy`/`WritePolicy`, `extract_memories_from_execution()`, `MemoryService.search/archive/cleanup_expired`, and `/api/v1/memories*`. The Python-cosine path is now augmented on PostgreSQL by the **pgvector** native index (`Migration 0015`): both live behind the same interface, and the native path is engaged only when the engine, column, vector width, and extension all check out — otherwise retrieval degrades seamlessly to the Python path, so callers never change.
...
- **Phase 1+ (Agent Runtime):** OpenAI and Anthropic adapters are now **real** (Chat Completions + Anthropic Messages, native SSE streaming, keyed via `OPENAI_API_KEY` / `ANTHROPIC_API_KEY`, keyless fallback), with a provider-prefix model-name protocol (`anthropic/claude-sonnet-5`). Remaining: Gemini / local adapters.
...
- **Memory:** embeddings are now **real** (`OpenAIEmbeddingProvider` via the shared `_client` transport, key via `OPENAI_API_KEY`, neutral-vector fallback) **and** PostgreSQL semantic search uses the **native pgvector index** (`Migration 0015`: `embedding_vector vector(1536)` + HNSW, written via `sync_vector`, ranked via native `<=>` with the Python-cosine path as the always-available fallback). Remaining: richer memory types (structured entities/relations) and working/conversation-scope memory management.
